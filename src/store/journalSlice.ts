...
export const fetchJournalEntries = createAsyncThunk<
  ...
  number,
  { rejectValue: string }
>("journal/fetchEntries", async (userId, { rejectWithValue }) => {
  try {
    const entries = await getJournals(userId);
    return entries;
  } catch (error: unknown) {
    return rejectWithValue(
      (error as Error).message || "Failed to fetch journal entries",
    );
  }
});
...
export const createJournalEntry = createAsyncThunk<
  ...
  { rejectValue: string }
>("journal/createEntry", async (data, { rejectWithValue }) => {
  try {
    const result = await createJournal(data);
    return result.entry;
  } catch (error: unknown) {
    return rejectWithValue(
      (error as Error).message || "Failed to create journal entry",
    );
  }
});
...
>(
  ...
  async ({ entry_id, title, content }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        (error as Error).message || "Failed to update journal entry",
      );
    }
  },
);
...
export const deleteJournalEntry = createAsyncThunk<
  ...
  number,
  { rejectValue: string }
>("journal/deleteEntry", async (entry_id, { rejectWithValue }) => {
  try {
    await deleteJournal(entry_id);
    return entry_id;
  } catch (error: unknown) {
    return rejectWithValue(
      (error as Error).message || "Failed to delete journal entry",
    );
  }
});
...
export const { resetJournal, clearJournal, setJournalEntries } =
  journalSlice.actions;
export const selectAllEntries = (state: { journal: JournalState }) =>
  state.journal.entries || [];
export const selectJournalLoading = (state: { journal: JournalState }) =>

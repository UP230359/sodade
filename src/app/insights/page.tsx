  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 md:py-12">
        <div className="mb-8">
          <p className="text-sm font-semibold tracking-wider text-primary uppercase mb-2">Professional Guidance</p>
        ...
        {deleteError && (
          <div className="mb-4 p-3 bg-anger/10 border border-anger/20 rounded-xl text-anger text-sm">
            {deleteError}
            <button
        ...
        <div className="bg-background rounded-2xl shadow-lg border border-muted overflow-hidden">
          {sortedInsights.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-sm text-secondary">No insights yet</p>
            </div>
          ) : (
            <div className="divide-y divide-muted">
              {sortedInsights.map((insight) => (
                <div
                  key={insight.id}
                  className="p-4 md:p-5 bg-foreground text-background"
                >
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h3 className="font-medium text-joy">
                          {insight.title}
                        </h3>
                      </div>
                      <p className="text-sm text-background/80 line-clamp-2 mt-0.5">
                        {insight.preview}
                      </p>
                    ...
                    <button
                      ...
                      disabled={isDeleting}
                      className="flex-shrink-0 p-1.5 rounded-lg hover:bg-anger/20 text-background/50 hover:text-anger transition-colors disabled:opacity-50"

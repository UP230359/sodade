...
export default function CommunityPage() {
  ...
  const handleSaveDraft = async (id: number) => {
    try {
      await updateReflection(id, {
        draft_recommendation: draftText,
  ...
  const handleSendInsight = async (reflection: Reflection) => {
    if (!reflection.draft_recommendation) return;

    try {
      await dispatch(createInsightEntry({
        checkin_id: reflection.checkin_id || 1,
        professional_id: professionalId,
        tag_id: null,
        content: reflection.draft_recommendation,
      })).unwrap();
      ...
      console.error("Error sending insight:", error);
    }
  };
                    ...
                    <div className="mt-3 space-y-3">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => handleSendInsight(reflection)} className="px-4 py-2 rounded-lg bg-[#007BFF] text-white hover:bg-[#007BFF]/90 transition-colors text-sm font-medium">
                            Send Insight
                          </button>
                        </div>
                      </div>

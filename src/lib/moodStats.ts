...
export type EmotionAccent = "joy" | "calm" | "sadness" | "anger" | "anxiety" | "neutral";
...
// a estos 4 valores. Si el backend algún día agrega una emoción nueva que no está aquí,
// no truena nada: metaFor() cae en DEFAULT_META más abajo.
const EMOTION_META: Record<string, EmotionMeta> = {
    joy: { accent: "joy", square: "bg-joy", chip: "bg-joy/20 text-joy border-joy", level: 5 },
    trust: { accent: "calm", square: "bg-pink-400", chip: "bg-pink-100 text-pink-600 border-pink-300", level: 4 },
    calm: { accent: "calm", square: "bg-calm", chip: "bg-calm/20 text-calm border-calm", level: 4 },
    surprise: { accent: "joy", square: "bg-orange-400", chip: "bg-orange-100 text-orange-600 border-orange-300", level: 3 },
    fear: { accent: "anxiety", square: "bg-anxiety", chip: "bg-anxiety/20 text-anxiety border-anxiety", level: 2 },
    sadness: { accent: "sadness", square: "bg-primary", chip: "bg-primary/20 text-primary border-primary", level: 2 },
    disgust: { accent: "neutral", square: "bg-neutral", chip: "bg-neutral/30 text-secondary border-neutral", level: 1 },
    anger: { accent: "anger", square: "bg-anger", chip: "bg-anger/20 text-anger border-anger", level: 1 },
};
...
export function getDominantEmotions(checkins: Checkin[]): {
    ...
    return { dominant: sorted[0]?.[0] ?? null, secondary: sorted[1]?.[0] ?? null };
}
...
export function getStreak(checkins: Checkin[]): number {
    ...
    const daysWithEntries = new Set(checkins.map((c) => new Date(c.created_at).toDateString()));
    let streak = 0;
...
export function getWeeklyPatterns(checkins: Checkin[]): WeeklyPattern[] {
    ...
    const thisWeekCount = checkins.filter((c) => new Date(c.created_at) >= startOfThisWeek).length;

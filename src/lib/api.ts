...
export const updateCheckin = async (checkinId: number, data: { note?: string; shared_anonymously?: boolean }): Promise<{ success: boolean }> => {
...
// Insights
export const getInsights = async (params?: { professional_id?: number; checkin_id?: number; limit?: number; user_id?: number }): Promise<Insight[]> => {
...
export const updateInsight = async (insightId: number, data: { content?: string; tag_id?: number | null }): Promise<{ success: boolean }> => {
...
// Reflections
export const getReflections = async (params?: { category?: string; sort?: "ASC" | "DESC" }): Promise<Reflection[]> => {
    const { data } = await api.get("/reflections", { params });
    return data;
};

export const updateReflection = async (reflectionId: number, data: { draft_recommendation?: string; tag?: string; status?: string }): Promise<{ success: boolean }> => {
